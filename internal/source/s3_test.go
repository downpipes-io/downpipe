...
// reads body bytes.
//
// WHY THE LIMIT IS INJECTED. This test used to stream the real 2 GiB ceiling through the
// test server and buffer it. That is what killed CI: every Build & Test leg and the
// coverage job died with SIGTERM at this package, deterministically, after the packages
// before it had passed. It survived locally, where there is far more memory than a hosted
// runner has, so main had been red on every run since June while the suite looked healthy
// on a developer machine. A small injected ceiling drives the identical code path, which
// is the same trade DirStore.maxBytes already makes for the same guard.
...
// TestS3StoreContentLengthBoundary is the authoritative pin (DP-M9) for the
...
// DP-M10: a key containing characters that require AWS URI-encoding must be
...
// TestS3StoreAddressesGoogleCloudPathStyle pins the claim cmd/downpipe/main.go:746 makes to an operator in
...
//
// WHY THIS TEST EXISTS. That claim is customer-facing text in a BREAK-GLASS tool, and until
// nothing graded it: zero test files in this module mentioned googleapis, and internal/source/ has no GCS
// implementation at all. The count reads exactly like the defect this campaign opened on ("0 azure refs vs
// 31 s3"), and it is NOT one, because Azure needed its own file for a reason azuresharedkey.go:16 states,
// "it is a different wire protocol", while R2 and GCS both speak S3.
//
// WHAT MAKES THE CLAIM TRUE, and it is the thing worth pinning rather than the endpoint string. GCS's
// S3-compatible XML API addresses a bucket PATH-STYLE, https://storage.googleapis.com/BUCKET/OBJECT, and
// this store builds every request that way unconditionally: three `"/" + s.bucket + "/"` sites and no
// bucket-in-host construction anywhere. So the claim holds by construction rather than by configuration,
// and the failure this test would catch is somebody adding virtual-host addressing, which would silently
// break GCS and R2 while leaving Amazon S3 working.
