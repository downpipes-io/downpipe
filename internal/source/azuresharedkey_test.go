...
// every signature this package produces is wrong, SURVIVED the whole suite. Every other assertion about
// the Authorization header is relational or structural: TestAzureSharedKeyAuthorizationHeader checks a
// shape regex, that a different request signs differently, and that a different key signs differently,
// and a consistently corrupted input satisfies all three. TestAzureSharedKeyMicrosoftWorkedExamples takes
// "got, _ :=" and grades the string to sign alone, discarding the authorization it is handed.
//
// So the canonicalisation was graded character for character and the HMAC over it was graded only for
// varying. The failure that leaves is a signer that is well shaped, request dependent, key dependent and
// wrong, which reaches an operator as 403 AuthenticationFailed on every call. On an Azure-only estate that
// is the break-glass path, so it fails at the moment it is needed and reads as a container permissions
// problem rather than as a signing fault.
//
