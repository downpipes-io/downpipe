...
// DP-L1: EncapsulateHybrid takes no caller reader and is always freshly randomised
...
// DP-L2: zeroize must overwrite key material with zeros and tolerate the empty and
...
// DP-L2: zeroizeArray must wipe the caller's fixed 32-byte key array through the
...
// DP-L2: the deferred zeroization of the KEM shared secret, wrap key and recovered
...
// DP-L3: ConstantTimeEqual must be length-safe. A length mismatch must report
