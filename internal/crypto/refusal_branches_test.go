...
//
// WHY THESE, AND WHY NOW. internal/crypto carries a 90% statement floor because it is the
// security-critical core of a binary a customer runs in a disaster, and it sat at
// 390 of 433 statements, exactly 0 statements above that floor: the next untested statement
// added to this package would have reddened main on an unrelated commit. Every one of the 43
// uncovered statements was an error or refusal branch, which is the half of this package a
// customer meets only when something has already gone wrong, and the half whose behaviour a
// recovery depends on being exact.
//
...
// message it expects.
//
// TWO BRANCHES ARE DELIBERATELY NOT HERE, because they are unreachable rather than untested, and
// both were put to the toolchain rather than assumed: ParseKEMPrivate rejects a bad X25519 half
// and a bad ML-KEM half, and neither rejection can be reached from an input of the right length.
// crypto/ecdh accepts an all-zero X25519 private scalar, and crypto/mlkem accepts an all-zero
// 64-byte decapsulation seed. Both return nil error, measured. The remaining uncovered branches
// are CSPRNG and AES key-schedule failures that have no seam short of injecting a failing
// primitive, which would be a seam added for a test rather than a property of the code.
