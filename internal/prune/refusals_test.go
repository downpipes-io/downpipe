...
// internal/prune sat 5 statements above its 85 per cent floor. It shares the shape
// internal/restore had: what was left uncovered is entirely the refusal half, the part an
// operator meets only when the destination or the receipt path is not what they assumed. This
// is the one command in the repo that deletes a customer's archives, so what it says when it
// declines to proceed is the whole of what the operator has to act on.
//
