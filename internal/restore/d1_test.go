...
// reader keeps both readings live: it does not choose which shape it is handed, only reads
// whatever is in the archive, and the operator guidance offers both shapes ("each
// <db>/00-header.sql ... or the single <db>.sql for a whole dump"). Dropping the bare-name
// reading would turn "appdb" into a directory-shaped key with nothing after the slash.
//
// So this is a decision about naming, and it is asserted as one: the bare name yields exactly
// one file, named for the database, holding the verified bytes unchanged.
...
// TestD1RestoresAsReadyToApplySQL proves the offline apply of a d1 record: it
