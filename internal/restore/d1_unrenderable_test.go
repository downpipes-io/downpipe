...
// The D1 format descriptor, and what it is now read for.
//
// spec.D1Descriptor.Format rides on every per-page D1 record the engine writes, is pinned in
// the conformance corpus, and until this change was READ FOR NO DECISION anywhere in this
// reader. These tests pin the decision it now makes, which is the one decision only it can
// make: the destination key is resolved by MakePlan before a single byte is decrypted, so the
// descriptor is the only thing that can stop a body the reader cannot render being planned to
// a .sql path.
...
// from the SAME constants it writes into the body (engine src/sources/d1.ts passes
// D1_HEADER_FORMAT, D1_ROWS_FORMAT and D1_SCHEMA_FORMAT as descriptor.d1Format, and
// src/format/writer-record.ts writes that to the manifest's d1.format), so a body and its
// descriptor disagree only for a writer that is not the engine. And the finding still fires:
...
// is authoritative, because two were in the repository and they disagreed.
//
// The engine is the only writer of the field, and it writes the SAME four constants into the
// body and into the record descriptor: engine src/sources/d1-format.ts exports
// D1_BACKUP_FORMAT "downpipe-d1-json/1", D1_HEADER_FORMAT "downpipe-d1-header/1",
// D1_ROWS_FORMAT "downpipe-d1-rows/1" and D1_SCHEMA_FORMAT "downpipe-d1-schema/1";
// src/sources/d1.ts passes those constants as descriptor.d1Format and src/format/
// writer-record.ts writes that to the manifest's d1.format. So the slash form is what rides on
// a real record in both places, and it is the form this reader matches on.
//
// The other spelling, "downpipe-d1-json-v1", exists once, as a fixture value in this
// repository's conformance corpus (internal/format/conformance_vectors_test.go, the
// d1-with-identity vector). No writer emits it, and that vector's expect.json does not assert
// it, so it pins bytes rather than a claim. It was left in place deliberately: the vector
// directories under internal/format/testdata/vectors are byte-identical to the engine's copies
// under engine/test/vectors, so regenerating one repository's corpus alone would redden the
// other. This test names it instead, so the disagreement is recorded rather than forgotten,
// and so the reader's reading of it is deliberate: it carries the family prefix and is not one
// of the four, so this reader treats it as a label it cannot render, which is exactly what it
// is.
...
// which is the text an operator acts on mid-recovery.
//
// Deleting the words "NOT SQL" from it left every other test in this change green: the reports
// were asserted for the record name and the label, and nothing looked at what the sentence
// actually told the operator to do. This is what looks.
//
