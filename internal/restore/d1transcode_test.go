...
//
// The reader's promise about a record's bytes is that it hands back what the engine put in,
// having hash-verified it. Transcoding is a courtesy laid on top of that promise for the shapes
// this reader can read, so anything else must pass through untouched: refusing an unrecognised
// body would mean a reader that cannot render a dump also refuses to give the operator their own
...
// TestAnUnknownDownpipeD1LabelIsSeparatedFromAForeignBody pins the requirement that an unknown
// format label must be judged by which of two causes it actually is, not treated as one thing.
//
// A default arm that reads every unrecognised label as "this was never a downpipe D1 body"
// conflates two causes with opposite operator responses:
//
...
//
// Conflating the two causes is a disaster in the second case: restored 3 record(s) ... 0 failed,
// exit 0, the record's verified JSON written to newerdb.sql, and D1ReplayGuidance printed above
// it telling the operator to run `sqlite3 restored.sqlite < <db>.sql`. Running that for real on
// the file the tool just wrote gives a "Parse error near line 1" over the JSON's opening brace,
// and exit 1, after the tool has already reported success.
//
...
// pins BOTH directions, because flagging the foreign body too would break the opaque-bytes
// contract a sibling test argues for at length.
func TestAnUnknownDownpipeD1LabelIsSeparatedFromAForeignBody(t *testing.T) {
	...
	// three things that would otherwise stay silent are all present: the record is named in the
...
// TestTranscodeD1RejectsSmuggledStatement guards a statement-smuggling regression: d1CreateTableRe
// and d1CreateSchemaRe are prefix matches with no end anchor, so a DDL/schema string that STARTS
// with the right keyword but carries a second statement after it must still be refused --
// never written verbatim next to the statement it claims to be. Covers all three call sites:
// writeCreateTable (header table DDL), transcodeD1Schema, and transcodeD1Full's schema phase.
...
// TestTranscodeD1RejectsBareKeywordSmuggle guards against a bypass of d1RequireSingleStatement's
// BEGIN/CASE...END depth tracker if it ran for every DDL/schema entry regardless of kind, keyed
// purely on a bare identifier spelled BEGIN/CASE/END with no grammatical context at all. A CREATE
// TABLE or CREATE INDEX entry -- neither of which can ever legitimately carry a BEGIN...END body
// -- could still open one with an ordinary unquoted column/expression name spelled "begin": the
// real top-level ';' right after it would hide behind depth>0, a later bare "END" would silently
// rebalance the counter back to zero, and the smuggled statement in between would sail through
// with no error. Both cases below (a CREATE TABLE and a CREATE INDEX variant, confirmed against
// the real sqlite3 CLI to show the DROP would actually execute) must be refused outright.
...
// above: SQLite does not reserve "begin" or "end", so a real single-statement CREATE TABLE using
// either as an ordinary column name (a shift-tracking table, say) is valid DDL -- confirmed
// against the installed sqlite3 (3.51.0) -- and must still transcode, not be rejected as an
// "unterminated BEGIN or CASE block" the way an unconditional depth tracker would. Each keyword
// is checked on its own (not both in the same table): together in one statement they would
// cancel out in a naive depth counter (one bare "begin" plus one bare "end" nets back to a
// balanced depth of zero) and mask the regression this guards against.
...
// TestTranscodeD1RejectsCrossEntryLiteralSplice guards against a second way an entry-by-entry
// single-statement check can be bypassed: validating each JSON array entry in isolation and
// silently accepting a quote/bracket/comment that opened but never closed, falling through to
// end-of-string with no error. Since every entry's SQL is concatenated into one buffer with only
// a plain ";\n" written in between (writeCreateTable, transcodeD1Schema, transcodeD1Full), one
// entry could leave an unterminated string literal dangling and a sibling entry could supply the
...
// TestTranscodeD1AcceptsMultilineTrigger is the positive control for the two guards above: a
// real, multi-line CREATE TRIGGER body -- as sqlite_master would actually store one, newlines and
// all, with internal ';'-separated statements and a nested CASE...END expression -- must still
// transcode unchanged. The strict single-statement check must not overcorrect into rejecting
// genuine trigger DDL just because it treats CREATE TABLE/INDEX/VIEW so much more strictly.
...
// TestTranscodeD1RejectsTriggerBodyAliasSmuggle guards a third way the bare BEGIN/CASE/END depth
// counter can be fooled, even when it is scoped down to CREATE-TRIGGER-only entries: a depth
// counter keyed on the bare spelling alone is still fooled inside a trigger body, since SQLite
// does not reserve "begin"/"end" there either. A derived-table column literally aliased "begin"
// bumps the same counter the trigger's own BEGIN needs, so the real closing END is never seen at
// depth zero and a smuggled statement rides through hidden behind it: piping the resulting text
// into sqlite3 per D1ReplayGuidance would execute the smuggled DROP (payments table gone
// afterward). Covers both the schema and full-body call sites.
...
// TestTranscodeD1AcceptsTriggerBodyReferencingBeginEndColumns guards a further regression shape: a
// real, single-statement trigger that references a column literally named "begin" or "end" -- not
// just declares one in CREATE TABLE, which the column-name test above already isolates -- must
// not be wrongly rejected as "an unterminated BEGIN or CASE block", which is what happens if the
// bare reference bumps the depth counter with no matching close. Both a bare reference and a
// NEW.-qualified one are covered, for both keywords individually (mirroring the CREATE TABLE
// test's own reasoning: combining both in one body would cancel out in a naive depth counter and
// mask the regression).
func TestTranscodeD1AcceptsTriggerBodyReferencingBeginEndColumns(t *testing.T) {
	cases := map[string]string{
		...
		"engine_review_poc": "CREATE TRIGGER shifts_ai AFTER INSERT ON shifts BEGIN INSERT INTO shift_log(shift_id, started) VALUES (NEW.id, NEW.begin); END",
	}
...
// TestD1TranscodeRoundTripsThroughSQLite is the keystone proof: a tiny synthetic D1 archive
// (header + a rows page carrying BLOB/NULL/bigint > 2^53/quoted-string cells + schema), each
// record a downpipe D1 JSON body, is restored through the real ApplyStreaming file path (which
// transcodes each to .sql), then the .sql files are loaded into a fresh database with the local
// sqlite3 and queried back. The recovered rows must equal what was backed up, proving the
// turnkey restore is correct end to end. Skips when sqlite3 is not installed so `go test` stays
// green on a host without it.
