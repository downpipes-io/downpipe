...
func TestDanglingSegmentsCountsDistinctAbsentPaths(t *testing.T) {
	...
	cases := []struct {
		...
		{
			// M7, a mutant that survived the first round: dropping the fs.ErrNotExist half
			// of the predicate left every case above green, because every fixture that
			// carried a SegmentReadError also carried fs.ErrNotExist. A segment GET that
			// was refused rather than answered is a read this reader could not complete,
			// not an object that is gone, and counting it here would send an operator to
			// fetch a copy of an object that is sitting where it should be.
...
func TestDanglingOnlyIsFalseWheneverAnythingFailedACheck(t *testing.T) {
	...
	cases := []struct {
		...
		{
			// The other half of the predicate, and the mutant that survived: a segment GET
			// that was REFUSED rather than answered. The object may well be there, so this
			// is not an incompleteness verdict and the harder reading has to stand.
