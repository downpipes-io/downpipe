...
// This file exists because of a single measured cause, not because of forty-one separate
// oversights. Every test in d1transcode_test.go drives the transcoder through the one
// top-level entry point transcodeD1 on a whole JSON body, and not one of them calls a leaf
// helper (writeCreateTable, writeInserts, d1Literal, d1TaggedLiteral, d1TriggerActionFollows)
// directly. The refusals those leaves carry are therefore only exercised when a whole-body
// document happens to reach them, and most never did: 41 of the package's 72 uncovered
// coverage blocks sat in this one file, and all of them were refusal branches, the half of
// the transcoder a customer meets only when the archive they are restoring in a disaster is
// not the shape the engine promised.
//
...
// TestD1LiteralGuardsNoArchiveBodyCanReach records a finding rather than only raising a
// number. Five refusals in d1Literal and d1TaggedLiteral cannot be reached through transcodeD1
// by any archive body at all. They are defence in depth against a future decoder change,
// exactly as d1NumRe's own comment says, and they are worth keeping: the cost is five lines
// and the thing they stop is a malformed token reaching an operator's .sql file as SQL.
//
// The mechanism was measured, and the first answer was wrong. The obvious reading is that
// encoding/json rejects the token while decoding the rows record, so the cell renderer never
// sees it. That is not what happens, and the difference matters to a customer. transcodeD1
