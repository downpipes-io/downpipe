...
// TestNoRedirectClientRefusesRedirect: the shared client returns a 3xx as the response and never
// requests the Location, so a Bearer on the first request is never replayed (ASVS V15.3.2).
func TestNoRedirectClientRefusesRedirect(t *testing.T) {
	...
	if redirected {
		t.Fatalf("the redirect target was requested: the Bearer would have been replayed")
	}
	// Control: the same server through a following client DOES reach the target, so the assertion
	// above discriminates the policy and not the server.
	following := &http.Client{Timeout: 5 * time.Second}
	ctrl, err := following.Get(srv.URL + "/start")
	if err != nil {
...
// TestNewClientNilDoerRefusesRedirect: NewClient's nil-doer fallback is the redirect-refusing
// client, never http.DefaultClient.
func TestNewClientNilDoerRefusesRedirect(t *testing.T) {
	c, err := NewClient("acct123", testToken, nil)
	if err != nil {
		t.Fatalf("NewClient: %v", err)
	}
	hc, ok := c.doer.(*http.Client)
	if !ok {
		t.Fatalf("fallback doer is %T, want *http.Client", c.doer)
	}
	if hc == http.DefaultClient {
		t.Fatalf("fallback doer is http.DefaultClient, which follows redirects")
	}
	if hc.CheckRedirect == nil {
		t.Fatalf("fallback doer has no CheckRedirect policy")
	}
	if err := hc.CheckRedirect(nil, nil); err != http.ErrUseLastResponse {
		t.Fatalf("CheckRedirect = %v, want http.ErrUseLastResponse", err)
	}
	if hc.Timeout == 0 {
		t.Fatalf("fallback doer has no timeout")
	}
}
