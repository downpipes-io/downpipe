...
func TestPreflightR2APIErrorIsFailedNotUnconfigured(t *testing.T) {
	// downpipe-src-005-05: a NON-transient R2 API error that is not the affirmative
	// "not enabled" answer (here a 400/code 7003) must FAIL with the redacted detail,
	// never be mis-attributed as "R2 not enabled" (which would mask a real outage as a
	// missing feature). This mirrors the zones/workers-plan/logpush error branches.
