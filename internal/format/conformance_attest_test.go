...
// already proves Open/StreamOpen agreement (conformance_test.go): without it, the corpus,
// the negative-vector mechanism and the refusal behaviour the "held to agreement" claim
// (MOAT.7, MOAT.9) rests on give the attest command zero automated coverage. A hand-walk
// of the compiled binary against 15 hand-picked vectors finds 14-of-15 exit-code parity
// with verify; replaying every one of the 49 archive vectors here, not a sample, finds a
// fuller shape: 33 vectors at exact parity, one
// (deleted-shard) independently caught by a different, still-correct gate, and 10 more that
// the 15-vector sample did not include, all legitimately outside Attest's documented scope
// (SPEC.md 8.8: attest never unwraps the master capsule, opens a shard manifest, decrypts a
// segment, or checks the recovery bundle). Every one of those 10 is named in
// attestParityOverrides below with the specific reason, not silently excluded — an
// unexplained exclusion is how a real gap hides.
//
...
//     an empty or shrunk corpus still refuses before any attest subtest would run (proven
//     live: emptying testdata/vectors names all 52 missing including deleted-shard, and
//     removing just deleted-shard alone still refuses by name).
...
//     that is not represented by any of the 49 vectors at all — proven live: the
//     canonnum.go case-collision RUNLOG bypass (a duplicate, re-cased JSON key defeating the
//     keyless --min-runlog-index self-consistency check) reintroduced locally and replayed
//     against this exact test does NOT fail it. Only the narrower unit tests (attest_test.go's
//     TestAttestKeylessRejectsCaseCollidingRunlogIndexEvenWithCanonicalKeyPresent and
//     canonnum_test.go's TestCaseCollidingDuplicateKeyIsRefusedEvenWithCanonicalKeyPresent)
//     catch it, because no committed corpus vector's RUNLOG contains a case-colliding
//     duplicate key. That is arguably the right home for it regardless — the corpus is
//     meant to be cross-implementation (SPEC.md 14.1), and this defect is an artefact of Go
//     encoding/json's case-insensitive Unmarshal specifically, not a byte-level format
//     property a second, non-Go reader would necessarily reproduce — but it means this
//     conformance coverage is real and load-bearing for the 44 vectors it exercises, and
//     is NOT a substitute for the unit-level regression tests that guard RUNLOG parsing
//     edge cases the corpus does not encode.
