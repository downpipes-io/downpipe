...
// chunkRange that disagrees with the sealed segment (DP-L4). A value spanning two STREAM
...
// derives the secrets file key (DP-L12). A 16-byte but malformed recordId such as
...
// caps the number of segments a record's chain declares (DP ML-10). restoreRecord takes
