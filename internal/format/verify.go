...
func unwrapMaster(root *spec.RootManifest, recipient *crypto.HybridKEMPrivate) ([]byte, error) {
	...
	// one it holds (KEY-2). These come from the root, whose signature is verified before
...
// held identity matches none of them (KEY-2).
...
// while no obtainable engine build stamps a two-component label. If one is ever published
// again, this branch is telling the holder of intact bytes that their manifest is damaged,
// which is the most expensive wrong thing it could say. A reader-pairing census over the
// update channel is the check that catches that case: it fails when the update channel
// offers a writer whose format label no reader ref implements.
//
