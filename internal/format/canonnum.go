...
func validateCounts(raw []byte, paths ...string) error {
	...
	for _, p := range paths {
		...
		// The case-collision check runs UNCONDITIONALLY, before the absence check below,
		// not only when the exact key is missing. It used to run only in the "absent"
		// branch, on the assumption that a case-colliding key only mattered when it was
		// the SOLE way encoding/json's later, case-insensitive Unmarshal could reach the
		// field. That missed the case where BOTH the canonical key and a re-cased
		// duplicate are present in the same object: resolvePath's exact-name lookup finds
		// and validates the canonical key (so this loop fell straight through to
		// checkCanonicalCount and passed), while encoding/json.Unmarshal walks the raw
		// object key-by-key and binds whichever of the two keys appears LAST in the byte
		// stream — attacker-controlled order, not spelling. So
		// {"index":1,"Index":999999} passed this check (validating the harmless "index":1)
		// while RunlogEntry.Index ended up 999999, silently defeating the keyless
		// --min-runlog-index self-consistency check the RUNLOG index guards (reproduced
		// live: attest --min-runlog-index 999 against exactly that RUNLOG line reported
		// max-index=999999 and exited 0). A key that case-collides with a normative field
		// is ambiguous input to a format whose field names are canonical, so it is refused
		// regardless of whether the canonical spelling is ALSO present.
