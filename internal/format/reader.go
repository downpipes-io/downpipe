...
type Options struct {
	...
	//
	// IT USED TO WAIVE TWELVE MORE THINGS, INCLUDING A SIGNATURE. It was the only
	// acknowledgement the freshness gate read, so a forged _RECOVERY/RUNLOG.sig, a
	// deleted RUNLOG, an unparseable or empty log, a run the log does not carry, an entry
	// contradicting the signed root, and a rewritten chain all opened the archive on the
	// strength of a customer typing a word that means "I know this run is old". Those are
	// two different permissions and one word granted both. They are now separate: see
	// AllowUnverifiedRunlog and FreshnessResult.RunlogUntrusted.
	AllowStale bool
...
// (engine/src/format/reader.go) so the offline DR restore and the online restore explain a
// rolled key the same way.
//
