...
// (SPEC.md 3, 9) and holds its bytes to the signature they were written under. Before it,
// 47 of the 49 archive vectors' bundles were read by nothing at all.
//
// Measured rather than assumed, at 6e152f5, by flipping one byte and running
// `go test -count=1 ./...` unpiped: a flip in FORMAT.md, RECOVER.md, SHA384SUMS or
// SHA384SUMS.sig under gzip-codec, seg-multi-chunk or master-capsule left the whole suite
// at exit 0, twelve times out of twelve. The same flip under seg-single-chunk DID fail,
// but only through FuzzReaderOpen, whose baseline loads that one vector with
// Options{CheckRecoveryBundle: true} as a positive control before fuzzing; no assertion
// anywhere named the bundle. The corpus's only other bundle reader is the
// recovery-bundle-tampered vector, which sets checkRecoveryBundle in its expect.json and
// asserts exit 2, and that assertion is satisfied by ANY unverifiable bundle: flipping a
// byte in its SHA384SUMS, and separately in its FORMAT.md, also left the suite at exit 0.
// So exactly one of the 49 bundles had any byte pinned, and that incidentally.
//
// The distinction this closes is the one the engine's own version of this gap turns on: the code path is
// covered, and well. bundle_test.go writes a bundle, verifies it and asserts a one-byte
// tamper fails; cmd/downpipe/bundle_attest_test.go drives the same through the restore
// command's --check-bundle flag. What was uncovered is the SHIPPED CORPUS, which is the
// artefact set an escrow recoverer and a clean-room re-implementer are pointed at
