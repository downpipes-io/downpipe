...
// TestAttestKeylessCatchesStrippedBreakGlass proves the CRYPTO-RSET-DROP-BREAKGLASS fix:
// a keyless attestation now runs the same recipient-set integrity gate keyed Open applies,
// so a dropped or role-flipped break-glass recipient — which strips the archive's offline
// recoverability — fails attest with ExitUnverified instead of green-lighting it rc=0. The
// fixture flips the only break-glass recipient's role to "operational" in the signed root and
// leaves the stale signature in place (a keyless attest never checks the signature), modelling
// a bucket-write adversary. It asserts parity with keyed Open under allow-unverified, where
// the signature is not the gate and Open's structural recipient-set check is the one that
// catches it — proving attest and Open now agree.
func TestAttestKeylessCatchesStrippedBreakGlass(t *testing.T) {
	...
	// Keyless attest must now reject it; before the fix it returned rc=0 because Attest never
	// ran the recipient-set gate.
	res, err := Attest(store, attestRunID, nil, 0)
	if err == nil {
...
// TestAttestSignerPinnedMinIndexCatchesTailRollback is the regression test for the bug
// this change fixes: checkRunlog's signer branch used to hardcode CheckFreshness's
// minIndex to 0, so attest --signer could never honour --min-runlog-index at all, even
// though verify/restore already do. buildArchive's RUNLOG carries exactly one entry (the run itself,
// index 1): a tail rollback, where an active bucket-write adversary deletes the newer
// runs and rolls the RUNLOG back to an older, wholesale validly-signed state, is
// internally self-consistent (the run IS the latest entry the truncated document
// contains) and has no in-document high-water mark to contradict it (SPEC.md 10). Only
// the out-of-band pin catches it. Before this fix, a signer-pinned attest against exactly
// this shape passed clean (verified live against the runlog-rollback conformance
// vector). With the fix, pinning above the log's real maximum index fails
// cryptographically anchored (RunlogVerified would have been true had checkRunlog not
// returned early); pinning at or below it still passes.
...
// integration-level regression for the canonnum.go case-collision fix
// (TestCaseCollidingDuplicateKeyIsRefusedEvenWithCanonicalKeyPresent): a keyless attest
// whose RUNLOG line carries both the canonical "index":1 and a re-cased duplicate
// "Index":999999 must fail, not pass with an inflated max-index. Before the fix,
// validateCounts only ran the case-collision check when the canonical key was ABSENT;
// with it present (validating cleanly), the duplicate slipped straight through to
// json.Unmarshal, which bound the LAST-occurring key in the object — the decoy — to
// RunlogEntry.Index. Reproduced live against the compiled binary before this fix: a
// keyless attest against exactly this RUNLOG line with --min-runlog-index 999 reported
// max-index=999999 and exited 0, defeating the keyless self-consistency check the pin
// is documented to provide, with no signature to forge at all.
func TestAttestKeylessRejectsCaseCollidingRunlogIndexEvenWithCanonicalKeyPresent(t *testing.T) {
	store := memStore{}
	_, _ = buildArchive(t, buildSpec{store: store, runID: attestRunID, dpID: "dp_attest", value: []byte("recover me"), secret: false, codec: spec.CodecNameNone})

	// Overwrite the RUNLOG with a hand-crafted line: the canonical "index":1 (which
	// validateCounts alone, pre-fix, would validate and wave through) followed by a
