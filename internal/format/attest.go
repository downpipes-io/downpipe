...
type AttestResult struct {
	...
	// detects a dropped or role-flipped break-glass recipient (CRYPTO-RSET-DROP-BREAKGLASS)
	// rather than green-lighting an archive whose offline recoverability has been stripped.
...
func Attest(store ObjectStore, runID string, signer *crypto.HybridVerifier, minIndex int64) (*AttestResult, error) {
	...
	// offline recoverability; keyed Open catches it (ExitUnverified), but a keyless attest
	// historically did not, so it returned rc=0 on an archive the break-glass key can no
	// longer open (CRYPTO-RSET-DROP-BREAKGLASS). This needs no master key and no signer: it
	// reads only the signed root's recipient list, fingerprints, wrap coverage and
