...
// than a blunt one.
//
// THIS TEST IS ONLY TRUE WHILE THAT RETIREMENT HOLDS. If an obtainable engine build ever
// stamps a two-component label again, this expectation inverts and the reader is telling
// the holder of intact bytes that their manifest is damaged. The check that catches that
// case is a reader-pairing census over the update channel, which fails when it offers a
// writer whose format label no reader ref implements.
