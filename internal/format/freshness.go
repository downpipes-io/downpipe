...
type FreshnessResult struct {
	...
	//
	// IT USED TO BE FALSE ON THE WORST INPUTS. Every early return in CheckFreshness
	// returned a bare FreshnessResult{}, so a forged RUNLOG signature and an absent RUNLOG
	// both left this false while a merely below-pin index set it true. Driven on a real
	// archive: with --allow-stale, a tampered RUNLOG.sig and a deleted RUNLOG each printed
	// "verified: run ... signature=valid" at exit 0 with the anti-rollback override warning
	// silent, and the same archive with a below-pin --min-runlog-index printed the warning.
	// The signal was inverted by severity, and this field is the one thing both the printed
	// warning and the signed receipt key on.
	...
	//
	// IT IS THE PERMISSION BOUNDARY, NOT A LABEL. --allow-stale is an age word, and it
	// used to waive all thirteen freshness failures: a forged _RECOVERY/RUNLOG.sig and a
	// deleted RUNLOG opened the archive on the strength of a customer typing a word that
	// says "I know this run is old". Options.acknowledges reads this field, so the twelve
	// findings the word never described now need --allow-unverified-runlog, which says
	// what it waives.
	//
	...
	//
	// WITHOUT IT THE OVERRIDE WARNING GUESSED, AND ON FIVE INPUTS IT GUESSED WRONG. The
	// unchecked warning named four causes ("absent, unreadable, unparseable, or its
	// signature did not verify"), and Unchecked is also set for an empty log, a run the log
	// does not carry, and an entry disagreeing with the signed root on downpipe, index or
	// prevRunId. Driven on a real archive with a validly signed RUNLOG whose entry read
	// index 7 against the signed root's 1, the operator was told the RUNLOG was absent,
	// unreadable, unparseable or unsigned, all four false, and was sent to recover a RUNLOG
	// that was intact. The reader knew the exact disagreement and printed a guess.
	//
...
func detectChainAnomaly(entries []spec.RunlogEntry) error {
	// LINE ORDER IS NOT LOAD-BEARING (SPEC.md 10, amended again after the
	// first production fleet): indices are allocated at trigger time but entries land
	// at FINALISE time, so concurrent runs legitimately append out of allocation order
	// (a 119-downpipe fleet produced exactly that within its first hour). The
