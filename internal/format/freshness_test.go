...
//
// This assertion used to read "--allow-stale proceeds despite the absent RUNLOG", which is the
// conflation the split removes: the flag is an age word, and an absent RUNLOG measures no age. It
// is one of only two places in the whole suite that encoded the old behaviour, and both were about
// an absent log. Nothing asserted that --allow-stale opened an archive whose RUNLOG SIGNATURE did
// not verify, which is the worse half of what it waived and the half no test could see.
...
// TestOpenMultiRunHistoryWithoutAllowStale is the cross-repo proof for RUNLOG-1: the offline reader
// restores a multi-run history WITHOUT --allow-stale. buildArchiveSpec(runs:2) produces exactly the
// shape the FIXED engine writes — the signed root's freshness.prevRunId is seeded from the same chain
// the RUNLOG records (engine seal/pipeline.ts + seal/slice.ts now derive it from destinationLocalPrev),
// so the latest run's root and its RUNLOG entry AGREE on prevRunId. The latest run must therefore Open
// clean (no AllowStale); the older run stays correctly flagged stale until AllowStale — the genuine
// anti-rollback behaviour the fix must NOT weaken (integrity != availability).
...
// TestCheckFreshnessRunlog1Divergence locks the RUNLOG-1 fix at the strict consumer: the freshness check
// passes ONLY when the signed root's freshness.prevRunId matches the RUNLOG entry's prevRunId, and the
// pre-fix divergence (a churn-minted root claiming prevRunId=nil while the RUNLOG links the real prior
// run) is rejected ExitStale on an otherwise-intact archive. The engine fix makes the engine ALWAYS
// produce the agreeing shape; this proves the reader still distinguishes the two so --allow-stale stays
// the deliberate waiver, not the default.
func TestCheckFreshnessRunlog1Divergence(t *testing.T) {
	...
	// BUG shape (pre-fix churn): the root claims prevRunId=nil while the RUNLOG links A. This is the
	// exact RUNLOG-1 disagreement that rejected intact backups with rc=5 until --allow-stale.
	...
	if err == nil {
		t.Fatal("a root/RUNLOG prevRunId divergence must be rejected (RUNLOG-1)")
	}
	var ee *ExitError
	if !errors.As(err, &ee) || ee.Code != ExitStale {
