...
func cmdPrune(_ context.Context, args []string) error {
	...
	if err != nil {
		...
		if errors.As(err, &ab) {
			...
			//
			// THE REMEDY NAMES THE MEASURED CAUSE, NOT THE COMMONEST ONE. This line used to close on
			// "Check that identity opens the run with downpipe verify before pruning again", which
			// asserts a cause the command never established. Driven on the conformance vector
