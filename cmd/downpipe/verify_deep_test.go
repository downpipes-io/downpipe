...
// (INT-1): a shallow `verify` over a good archive succeeds but its one-line verdict no
...
// TestVerifyDeepCatchesTamperedSegment is the headline INT-1 regression: against an archive
...
// TestRestoreDiscardExitCodeIsCoded proves the swallowed-exit-code fix (INT-1/INT-4) at the
