...
//
// Guidance telling an operator to keep identity.key offline "on a printed recovery sheet" would
// tell them to copy a KEM private key onto the one artefact the product designs to be safe on
// paper. The console prints fingerprints there under a heading saying those values are public
// and safe to record, so guidance naming a private key file and the recovery sheet together
// with no negation would disagree with the console about where a private key lives, and the
// reader's side would be the unsafe one.
//
...
//
// Asserting only non-zero would equally be satisfied by exit 1, and by exit 2, the code that
// means an archive failed to verify, which is strictly worse: an operator who mistypes a flag
// and reads 2 has been told their archive is bad. Pinning ExitUsage exactly is what makes the
// exit code mean something specific. TestFlagParseOutcomesCarryTheRightExitCode pins this for
// every dispatched command; this one keeps its own case because it is what somebody reads when
// keygen changes.
...
func TestDescribeIdentity(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			// values under the wrong labels and every single-field case above stays green, so
			// this is the case that catches a database<->account value swap in describeIdentity
			// (proven: this case alone fails against the swap while every case above it still
			// passes).
