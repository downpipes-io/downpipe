...
// This is the KEY-1 index that answers "which offline key opens which runs". Without a
// --signer the summary line must say so plainly (ML-26): the index is unauthenticated
...
// reports "signer-verified" rather than the unverified caveat (ML-26).
...
// TestCmdKeysWhichWithWrongSignerFailsClosed is the ML-26 regression: a --signer that does
...
// TestCmdKeysWhichSkipsRunWithForgedRootSignature is the per-run half of the ML-26 fix: a
