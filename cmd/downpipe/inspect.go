...
//
// It opens with AllowStale, and format.Open runs the freshness check on every open and hands
// the result back on the reader, so inspect always holds a verdict on this. Staying silent
// about it would mean a forged _RECOVERY/RUNLOG.sig produces the same output, at exit 0, as
// an intact archive: silence is defensible only if a reader cannot mistake this command for a
// check, and a customer who passes --identity and --signer and is shown the decrypted detail
// has just had the root signature, the master capsule, the key commitment, the recipient set,
// the record count and the Merkle root all genuinely verified. What they conclude from a
// clean-looking dump at exit 0 is that the archive checked out, and on the one thing inspect
// would otherwise withhold they would be wrong.
//
...
func cmdInspect(args []string) error {
	...
	if !idf.supplied() {
		...
		// The closing instruction is CONDITIONAL, because on a run this reader cannot open, an
		// unconditional "Run: downpipe verify" would contradict the format line six lines above it,
		// which already says verify and restore will refuse this run: an operator following an
		// unconditional next step would only get exit 6. Naming a next step that this same output
		// has already said will fail is not a next step.
...
// TestDescribeIdentity's "database and account together" case).
...
//
// A line that only echoed the archive's self-description would print identically whether the
// reader can actually read the archive or not: for the conformance vector unknown-major
// (formatVersion downpipe/9.0.0, a version this reader refuses at exit 6), that would produce a
// full, clean-looking inspect block at EXIT 0 with no mention anywhere that the reader cannot
// read it, closing on a line telling the operator to run downpipe verify, which then refuses.
// inspect is a diagnostic viewer and is deliberately not made to refuse, which is the right
// call; instead it says what it already knows. This is the same rule the freshness line above
// follows: a line that appears only on trouble is one whose absence an operator cannot rely on,
// so the reader's support is stated on the readable case too.
