...
# verdict or the guard forces a non-zero exit. This gate is the reason the shell guard exists. The `awk`
# note further down records the run where a `set -e` termination made the summary line unreachable and
# the job reported nine rows and no verdict.
...
if [[ "${1:-}" == "--self-test" ]]; then
  ...
  check "a package count under the floor declares a skip rather than a verdict" "$floor_out" "VERDICT SKIPPED" 1
...
if [[ ! -s "$profile" ]]; then
  ...
  verdict_skipped "no coverage profile at $profile, so no package was measured" require
  exit 1
fi
...
if [[ $listed -lt $package_floor ]]; then
  ...
  verdict_skipped "go list reported $listed package(s), under the floor of $package_floor" require
  exit 1
fi
...
if [[ $missing -ne 0 || $awk_status -ne 0 ]]; then
  ...
  verdict_reached $(( missing + awk_status )) "$listed"
  exit 1
fi
echo "coverage-gate: every package meets its floor"
verdict_reached 0 "$listed"
