...
#
# WHY THIS EXISTS, measured rather than argued. downpipe main went red at
# 39d51ea3 on `Fuzz (./internal/restore)` and stayed red for over an hour. The whole failure
# was two lines:
#
...
# Named function rather than a trap: installing our own EXIT trap would REPLACE the guard's and
# silently disarm it, which scripts/verdict-guard-gate.mjs treats as a finding.
verdict_guard_cleanup() { rm -rf "$tmpdir"; }
...
if [[ "${1:-}" == "--self-test" ]]; then
  ...
  # green by a mutation, including one that counted no failing target and reported VERDICT: PASS
  ...
  cp -R "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/lib" "$mp/scripts/lib"
...
GO_SHIM
  ...
  assert_eq "1" "$(grep -cF 'VERDICT: FAIL' "$mp/out" || true)" \
...
if [[ "$#" -lt 3 ]]; then
  ...
  verdict_skipped "called with $# argument(s); a package, a budget and at least one target are required" require
  exit 1
fi
...
if [[ ! -d "$repo_root/$pkg_dir" ]]; then
  verdict_skipped "no package directory at $pkg_dir, so no target could be fuzzed" require
  exit 1
fi
...
verdict_reached "$failures" "$attempts"
