...
if [[ -z "$doc_list" ]]; then
  ...
  verdict_skipped "could not extract the VECTOR CORPUS block from $conformance_md, so no name was compared" require
  exit 1
fi
...
if [[ -z "$spec_list" ]]; then
  ...
  verdict_skipped "could not extract any enumerated vector from $spec_md, so the SPEC.md half compared nothing" require
  exit 1
fi
