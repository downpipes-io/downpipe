...
reader is telling the holder of intact bytes that their root manifest is damaged. The check
that falls in that case is `scripts/reader-pairing-census.mjs` in the harness repository,
which fails when the update channel offers a writer whose format label no reader ref
implements.
