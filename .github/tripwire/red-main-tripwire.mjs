...
// the repo it lands in. What eventually found it was a scheduled sweep
// looking at something else, which means the thirty-eighth red was the one that got noticed rather
...
const ORDER_FIXTURES = [
  ...
  // a standing red belonging to another one. A11y Axe, one of the docs repo's workflows, is paths-filtered
  // and is often the only one a push there runs.
  ...
  // false`, so it concludes `skipped` on every run while a repo is private, and the streaks are long. The
  // current unbroken run of skips is 54 on control-plane, 82 on downpipe, at least 198 on
  // docs where the count is bounded by the page read rather than by a success, and none at all on website,
  // which has no `scorecard.yml` and so has no such streak in either direction. A naive "latest run was not
  // success" reader would call every one of the skips a red main. The repos are named individually rather
  // than written as "this repo" because this file is copied into all of them, and a count measured in one
  // is not a fact about the others: an earlier copy of this comment stated a streak of 14 as a fact about
  // "this repo" and was wrong in every repo it was read in, including the one it was measured in.
